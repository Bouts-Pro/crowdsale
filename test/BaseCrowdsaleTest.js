...
const Crowdsale = artifacts.require('./BoutsCrowdsale.sol');
const Token = artifacts.require('./PreBOUToken.sol');

const tokenDecimals = 18;
const tokenDecimalsIncrease = new BigNumber(10).pow(tokenDecimals);
const boutForEth = new BigNumber(3000);
const rate = boutForEth;  // rate - 1.000.000.000.000.000.000.000.000(one ETH) to 3000(CAT)
contract('Crowdsale: ', function ([_, wallet, boutsProWallet, walletForMint, walletInvestorFirst, walletInvestorSecond, walletMetaMask]) {
    ...
    let boutToUsedPrice = new BigNumber(0);
    ...
    before(async function () {
        ...
        startTime = latestTime() + duration.seconds(200);
        endTime = startTime + duration.hours(5);
        afterWhitelistTime = startTime + duration.hours(4);
        afterEndTime = endTime + duration.seconds(1);

        crowdsale = await Crowdsale.new(startTime, endTime, rate, wallet, wallet, boutsProWallet);
        tokens = Token.at(await crowdsale.token.call());
        bonusCoefficient = await crowdsale.BONUS_COEFF.call();
        boutToUsedPrice = await crowdsale.TOKEN_USDCENT_PRICE.call();
    ...
    it('funds on wallets', async function () {
        let boutsProWalletTokens = await crowdsale.BOUTSPRO_AMOUNT.call();
        await validateBalance(boutsProWallet, boutsProWalletTokens);
    });
    ...
    it('buy tokens for test count in wallet via MetaMask (via buyTokens)', async function () {
        ...
        await validateBalance(walletMetaMask, initialWalletMMBalance.add(new BigNumber(boutForEth)
    ...
    it('buy tokens at 45 - 60 days. transfer to other wallet', async function () {
        ...
        await validateBalance(walletForMint, initialWalletMintBalance.add(new BigNumber(boutForEth)
            .mul(tokenDecimalsIncrease)));

        await validateBalance(walletInvestorSecond, investorBalance);
    });

    it('change owner to BoutsPro wallet', async function () {
        await mintTokensWithValidateBalance(walletForMint, 1);

        const owner = await tokens.owner();
        owner.should.equal(crowdsale.address);

        await crowdsale.transferOwnership(boutsProWallet);

        await mintTokens(walletForMint, 1).should
            .be
            .rejectedWith(EVMRevert);

        await mintTokensWithValidateBalance(walletForMint, 1, {from: boutsProWallet});
    });
    ...
    it('validate totalSupply of tokens', async function () {
        ...
        let boutsProWalletTokens = await crowdsale.BOUTSPRO_AMOUNT.call();
        let fullSupply = boutsProWalletTokens.add(usedTokensSupply);
    ...
    it('finalize crowdsale', async function () {
        ...
        await crowdsale.finalize({from: boutsProWallet});

        await mintTokens(walletForMint, 1,  {from: boutsProWallet}).should
            .be
            .rejectedWith(EVMRevert);
    });
    ...
    it('is boutsProWallet owner of BOUToken', async function () {
        const owner = await tokens.owner();
        owner.should.equal(boutsProWallet);
    });
